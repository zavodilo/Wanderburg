...
// ----------------------------------------------------------------------------
//  node tools/manifest.mjs            regenerate js/core/SceneSchema.js
//  node tools/manifest.mjs --check    fail if js/core/SceneSchema.js drifts
//
...
//  kinds and the semantic Scene API surface (js/core/SceneAPI.js). The game reads the same
//  canon at runtime (SCENE_SCHEMA), so an agent can validate a scene edit BEFORE
//  rendering it.
//
//  Generated file: js/core/SceneSchema.js (a classic script, like everything in the kit).
// ============================================================================
...
const OUT = path.join(ROOT, 'js', 'core', 'SceneSchema.js');
...
// surface (js/core/SceneAPI.js). Canon: js/Constants.js + _utils/editor/schema.js.
...
if (CHECK) {
    ...
    if (have !== body) {
        console.error('manifest drift: js/core/SceneSchema.js — run node tools/manifest.mjs');
        process.exit(1);
    }
    console.log('manifest: ok (' + constants.length + ' constants, schema v' + schema.schemaVersion + ')');
} else {
    fs.writeFileSync(OUT, body);
    console.log('manifest: js/core/SceneSchema.js (' + constants.length + ' constants, schema v' + schema.schemaVersion + ')');
}
