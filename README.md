...
сборки). Звук и музыка **синтезируются кодом** (36 файлов, `tools/make-wander-sounds.mjs`).
Геометрия — либо процедурная (`js/WanderMesh.js`), либо **бесплатные модели CC0**: флора долины,
шатры деревень, надвратные башни вардена и знамёна корпусов запекаются из пакетов Kenney
*Nature Kit* и *Castle Kit* (CC0, `assets/models/pack/`, лицензии приложены) в
`js/WanderPackGeo.js` инструментом `tools/make-pack-geo.mjs` — пак-модель становится данными
батчера, поэтому 340 деревьев по-прежнему стоят 32 draw call. Нет пака — игра молно остаётся
на процедурной геометрии (инвариант набора «отсутствующий ассет не роняет сцену»).
...
node verify/input-live.mjs       # те же клавиши в ЖИВОМ цикле кадров (нужен GPU-хост; без GPU честно пропускается)
...
* `tools/make-pack-geo.mjs` — запекание CC0-моделей (`assets/models/pack/*.glb`) в геометрию
  батчера (`js/WanderPackGeo.js`); `--check` сторожит дрейф.
